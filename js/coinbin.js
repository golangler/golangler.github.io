		...
		//if(email.match(/[\s\w\d]+@[\s\w\d]+/g)){
			...
			if($("#openPass").val().length>=1){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					//pass = pass+' evbox';
					/*alert(pass);*/
					s += ' '+pass+' '+"1Auoot7u5phrpamMvByrkNQWZNueJtxC8a";
					//s += ' '+pass;
					//alert(s);
					//s += s.length+'|!@'+((pass.length*7)+email.length)*7;*/
					s += s.length+'|!@'+((pass.length*3)+email.length)*2;
					//alert(s);
				...
				$("#openLoginStatus").html("Your password must be at least 1 chars long").removeClass("hidden").fadeOut().fadeIn();
			}
		} else {
